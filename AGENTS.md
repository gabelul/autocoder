...
- Mission Control activity feed is stored in `activity_events` (project DB) and surfaced in the Web UI (shortcut `M`). Retention: `AUTOCODER_ACTIVITY_KEEP_DAYS`, `AUTOCODER_ACTIVITY_KEEP_ROWS` (or UI Advanced → Logs).
