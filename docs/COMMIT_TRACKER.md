...
| `94e0b05` | Token/perf/security refactor sweep | **Already have (selectively)** | We already use per-session `allowed_tools` (agent/spec/assistant/expand/QA), have retry backoff+jitter, and hide Playwright in YOLO mode. Remaining ideas worth re-checking later: lower coding agent `max_turns` (ours is 1000 in `src/autocoder/agent/client.py`), and upstream’s `EXTRA_READ_PATHS` pattern (security-sensitive). |
...
| `24481d4` | Headless browser toggle to settings UI | **Not planned (for now)** | Could be added later via `ProjectRuntimeSettings` → env `PLAYWRIGHT_HEADLESS` → append `--headless` to `@playwright/mcp` args; current default is headed + `--isolated`. |
