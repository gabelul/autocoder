...
| `94e0b05` | Token/perf/security refactor sweep | **Ported (selectively)** | We already use per-session `allowed_tools` (agent/spec/assistant/expand/QA), have retry backoff+jitter, and hide Playwright in YOLO mode. We also lowered default coding agent `max_turns` (1000 → 300; override with `AUTOCODER_AGENT_MAX_TURNS`). Remaining idea worth revisiting: upstream’s `EXTRA_READ_PATHS` pattern (security-sensitive). |
...
| `24481d4` | Headless browser toggle to settings UI | **Ported (adapted)** | Added per-project runtime setting `playwright_headless` (Settings → Run → Project Runtime). When enabled, injects `PLAYWRIGHT_HEADLESS=1` and starts `@playwright/mcp` with `--headless`. Default remains headed + `--isolated`. |
