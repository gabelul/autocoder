...
| `bf3a6b0` | Per‑agent log viewer + stuck agent fixes + scheduling score | **Mostly covered** | We already have WorkerLogsPanel, gatekeeper artifacts, retry/backoff, and worktree cleanup. If we want per‑agent modal logs, add on top of existing logs. |
