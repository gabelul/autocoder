...
def enhance_prompt_with_knowledge(prompt: str, feature: dict) -> str:
    """
    Enhance a prompt with examples from the knowledge base.

    Finds similar past features and adds their implementation patterns
    as reference examples to the prompt.
    ...
    Returns:
        Enhanced prompt with reference examples appended
    """
    try:
        from knowledge_base import get_knowledge_base

        kb = get_knowledge_base()
        reference = kb.get_reference_prompt(feature)
        ...
        # If knowledge base fails, just return original prompt
        print(f"Warning: Could not enhance prompt with knowledge base: {e}")
        return prompt
