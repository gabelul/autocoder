...
def create_client(
    ...
    *,
    ...
    Authentication:
    - Prefers ANTHROPIC_AUTH_TOKEN + ANTHROPIC_BASE_URL (custom endpoints)
    - Falls back to Claude CLI credentials from ~/.claude/.credentials.json
    """
    # Check for custom API configuration (proxies, third-party services)
    # Map ANTHROPIC_AUTH_TOKEN to ANTHROPIC_API_KEY for SDK compatibility
    use_custom_api = False
    if "ANTHROPIC_AUTH_TOKEN" in os.environ:
        os.environ["ANTHROPIC_API_KEY"] = os.environ["ANTHROPIC_AUTH_TOKEN"]
        use_custom_api = True
        print("Using custom API authentication (ANTHROPIC_AUTH_TOKEN)")
        if "ANTHROPIC_BASE_URL" in os.environ:
            print(f"Custom API endpoint: {os.environ['ANTHROPIC_BASE_URL']}")
    ...
    # - Custom API: Use ANTHROPIC_AUTH_TOKEN (no CLI needed)
    # - Official API: Use Claude CLI credentials
    if use_custom_api:
        print("   - Authentication: Custom API token (no CLI required)")
        ...
        if system_cli:
            ...
            cli_path = None
            print("   - Warning: System Claude CLI not found, may use bundled CLI")
    print()
    ...
    return ClaudeSDKClient(
        options=ClaudeAgentOptions(
            model=model,
            cli_path=cli_path,  # None for custom API, system CLI otherwise
