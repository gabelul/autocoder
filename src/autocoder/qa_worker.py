...
def _read_feature_plan(repo: Path, *, max_chars: int = 6000) -> str:
    """
    Read the optional per-feature plan artifact generated by the orchestrator.

    This is written into the agent worktree under `.autocoder/feature_plan.md` and is passed to
    workers via `AUTOCODER_FEATURE_PLAN_PATH`. Patch workers don't use the full Claude agent loop,
    so we explicitly inject this plan into their prompts here.
    """
    candidates: list[Path] = []
    env_path = str(os.environ.get("AUTOCODER_FEATURE_PLAN_PATH", "")).strip()
    if env_path:
        candidates.append(Path(env_path))
    candidates.append(repo / ".autocoder" / "feature_plan.md")
...
def _fix_prompt(*, repo: Path, project_dir: Path, failure: str, diff: str, attempt: int) -> str:
